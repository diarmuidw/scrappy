 ...
 */
var logEnabled = false; /** ************** Inheritence *************** */
...
Function.prototype.inherit = function(parentClassOrObject, includeConstructorDefs) {
...
Clip.prototype.hasBody = function() {
    return (this.window && this.window.document && this.window.document.body && this.window.document.body.tagName.toLowerCase() == "body");
};
...
Clip.prototype.clipBody = function() {
    if (!this.hasBody()) {
        if (logEnabled) console.log("Document has no body...");
        return false;
    }
    var s = 0;
    var e = 0;
...
Clip.prototype.clipSelection = function() {
    if (!this.hasSelection()) {
        if (logEnabled) console.log("No selection to clip");
        return false;
    }
    if (Clip.constants.isIE) {
...
Clip.prototype.serializeDOMNode = function(node, parentNode) {
    ...
    if (this.range && !this.rangeIntersectsNode(node)) {
        if (logEnabled) console.log("Skipping serialization of node: " + node.nodeName + " cuz it's not in range...");
        return str;
    }
    if (!this.keepNode(node)) {
        if (logEnabled) console.log("Skipping seralization of node: " + node.nodeName + " cuz it's a no-keeper");
        return str;
    }
    if (logEnabled) console.log("SerializeDOMNode: " + node.nodeName);
    if (node.nodeType == 3) { // Text block
        if (logEnabled) console.log("Serializing text node...");
        if (this.range) {
            if (this.range.startContainer == node && this.range.startContainer == this.range.endContainer) {
                ...
                str += this.constructor.HTMLEncode(node.nodeValue.substring(this.range.startOffset));
            } else if (this.range.endContainer == node) {
                str += this.constructor.HTMLEncode(node.nodeValue.substring(0, this.range.endOffset));
        ...
        if (this.range && this.range.commonAncestorContainer == node && this.range.startContainer != this.range.commonAncestorContainer && !this.isListNode(node)) {
            if (logEnabled) console.log("Ignoring range ancestor: " + node.nodeName);
        } else {
            // serialize node
            if (logEnabled) console.log("Serializing node: " + node.nodeName);
            ...
            if (this.stylingStrategy) {
                if (logEnabled) console.log("Styling node: " + node.nodeName);
                var nodeStyle = this.stylingStrategy.styleForNode(node, parentNode);
                if (logEnabled) console.dir(nodeStyle);
            ...
            if (!node.hasChildNodes() && this.isSelfClosingNode(node)) {
                if (logEnabled) console.log("Closing self-closing tag: " + node.nodeName);
                str += "/>";
            } else {
                str += ">";
            }
        ...
        if (node.nodeName.toLowerCase() != "iframe" && node.hasChildNodes()) {
            ...
            for (var j = 0; j < children.length; j++) {
                ...
                if (child != null && child.nodeType > 0 && child.nodeName != 'SCRIPT' && child.nodeName != 'IFRAME') {
...
Clip.prototype.keepNodeAttr = function(attrName) {
    return (typeof attrName == 'string' && typeof Clip.NOKEEP_NODE_ATTRIBUTES[attrName.toLowerCase()] == 'undefined');
};
Clip.prototype.keepNode = function(node) {
    if (node) {
        if (node.nodeType == 3) {
            ...
            if (node.nodeName.indexOf('#') == 0 || !this.isNodeVisible(node)) return false;
...
Clip.prototype.isSelfClosingNode = function(node) {
    return (node && typeof Clip.SELF_CLOSING_NODE_NAMES[node.nodeName.toLowerCase()] != 'undefined');
};
Clip.prototype.isListNode = function(node) {
    return (node && node.nodeType == 1 && typeof Clip.LIST_NODE_NAMES[node.nodeName.toLowerCase()] != 'undefined');
};
...
Clip.prototype.showClipperPanel = function() {
    ...
    view.innerHTML = '<iframe id="e_iframe" ' + ' onLoad="p = document.getElementById(\'e_data\'); if (p && p.style) {c = p.style.zIndex; if (c==7) {p.parentNode.parentNode.removeChild(p.parentNode);} p.style.zIndex = ++c;}" ' + 'name="e_iframe" src="' + this.clipperURL + '/loadingClip.html" scrolling="no" frameborder="0" style="width:100%; height:100%; ' + 'border:1px; padding:0px; margin:0px;"></iframe>';
...
Clip.prototype.scrollTop = function() {
    ...
    this.window.pageYOffset ? this.window.pageYOffset : 0, this.window.document.documentElement ? this.window.document.documentElement.scrollTop : 0, this.window.document.body ? this.window.document.body.scrollTop : 0);
};
...
ClipStyle.prototype.addStyle = function(style) {
    if (CSSStyleDeclaration && style instanceof CSSStyleDeclaration && style.length > 0) {
        ...
        for (var i in style) {
            ...
            if (typeof style[i] != 'function' && typeof this.constructor.prototype[i] == 'undefined') {
...
ClipStyle.prototype.removeStyle = function(style, fn) {
    ...
    function rem(prop, value) {
        if (typeof self[prop] != 'undefined' && typeof self.constructor.prototype[prop] == 'undefined' && (typeof fn == 'function' || self[prop] == value)) {
...
ClipStyle.styleInArray = function(style, styleArray) {
    ...
    var styleType = ((i = style.indexOf("-")) > 0) ? style.substring(0, i).toLowerCase() : style.toLowerCase();
...
ClipStyle.prototype.deriveStyle = function(style, matchStyle, keepArray) {
    this.removeStyle(style, function(prop, propValue, value) {
        if (keepArray instanceof Array && ClipStyle.styleInArray(prop, keepArray)) return false;
...
ClipStyle.prototype.toString = function() {
    ...
    if (this.length > 0) {
        for (var i in this) {
            if (typeof this[i] != 'string' || typeof this.constructor.prototype[i] != 'undefined' || this[i].length == 0) continue;
            str += i + ":" + this[i] + ";";
        }
    }
    return str;
...
ClipStyle.getNodeStyle = function(node, computed, filter) {
    ...
    if (logEnabled) console.log(">>> NODE: " + node.nodeName + "/" + node.nodeType);
    if (node && typeof node.nodeType == 'number' && node.nodeType == 1) {
        ...
        if (computed) {
            ...
            if (logEnabled) console.log(">>> Getting matched rules");
    ...
    if (logEnabled) console.log(">>> " + node.nodeName + " style: " + style.toString());
    return style;
}; /** ************** SelectionFinder *************** */
/**
...
SelectionFinder.prototype.hasSelection = function() {
    ...
    if (range && (range.startContainer != range.endContainer || (range.startContainer == range.endContainer && range.startOffset != range.endOffset))) {
        return true;
    }
    return false;
};
...
SelectionFinder.prototype._findSelectionInDocument = function(doc, deep) {
    ...
    if (sel && !hasSelection && deep) {
        if (logEnabled) console.log("Empty range, trying frames");
        var nestedDocs = this.findNestedDocuments(doc);
        if (logEnabled) console.log("# of nested docs: " + nestedDocs.length);
        if (nestedDocs.length > 0) {
            for (var i = 0; i < nestedDocs.length; i++) {
                if (nestedDocs[i]) {
                    if (logEnabled) console.log("Trying nested doc: " + nestedDocs[i]);
...
ClipTextStylingStrategy.prototype.isFormatNode = function(node) {
    return (node && node.nodeType == 1 && typeof ClipTextStylingStrategy.FORMAT_NODE_NAMES[node.nodeName.toLowerCase()] != 'undefined');
};
...
ClipFullStylingStrategy.prototype.styleForNode = function(node, parentNode) {
    ...
    if (node && node.nodeType == 1) {
        ...
        if (parentNode) {
            if (logEnabled) console.log("Deriving style...");
